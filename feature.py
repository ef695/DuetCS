...
def add_record(node_list,t_lang,abs_path_list,pathnum_list,pathtoken_list,pathtype,pathtypefile,pathtypelist,pathtokenlist,pathtypelist_file,pathtokenlist_file):	
	record={}
	path_list=[]
	...
	for i in range(ll):
		...
		if is_exsit(cur_path):
			...
			if path in pathtype["path"]:
				...
				pathtypelist.append(path_name)
				pathtype["path"].append(path)
				pathtype["name"].append(path_name)
				pathtype["amount"]+=1
				#print(pathtype)
				f1=open(pathtypefile,"w")
				json.dump(pathtype,f1)
				f1.close()
				f2=open(pathtypelist_file,"w")
				json.dump(pathtypelist,f2)
				f2.close()
			...
			for t in token:
				...
				token_depend.append(token_name)
				if token_name not in pathtokenlist:
					pathtokenlist.append(token_name)
					f3=open(pathtokenlist_file,"w")
					json.dump(pathtokenlist,f3)
					f3.close()
			record[path_name].append(token_depend)	
	...
	for sl in lang_collection:
		if s_lang==sl:
			...
			for t_lang in lang_collection_tmp:
				...
				f2=open(t_lang+"_"+s_lang+"_pt.json","r")
				pathtypelist=json.load(f2)
				f2.close()
				pathtypelist_file=t_lang+"_"+s_lang+"_pt.json"
				f3=open(t_lang+"_"+s_lang+"_tk.json","r")
				pathtokenlist=json.load(f3)
				f3.close()
				pathtokenlist_file=t_lang+"_"+s_lang+"_tk.json"
				fp=add_record(node_match_list,t_lang,abs_path_list,pathnum_list,pathtoken_list,pathtype,pathtypefile,pathtypelist,pathtokenlist,pathtypelist_file,pathtokenlist_file)
				prog={"file":sys.argv[1],"feature":fp,"feature_vector":[], "ve":0}
